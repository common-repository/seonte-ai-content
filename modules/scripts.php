...
if( !class_exists('sacasAddStylesSync') ){
	class sacasAddStylesSync{
		/* V1.0.2 */
		...
		public function process_enq_line( $line ){
			...
			$custom_id  = wp_rand( 1000, 9999).basename( $line['url'] );
			if( $line['type'] == 'style' ){
				wp_enqueue_style( $this->plugin_prefix.$custom_id, $line['url'] ) ;
			}
			if( $line['type'] == 'script' ){
				
				$rand_prefix = wp_rand(1000, 9999);
				if( isset( $line['id'] )  ){
...
add_Action('init', function(  ){
	...
	$insert_script = new sacasAddStylesSync( 'sacas' , $scripts_list);
})
